...
class portable_iarchive : public portable_iprimitive
	...
	void init(unsigned flags) {
		...
		if (flags & no_header) set_library_version(input_library_version);
...
public:
	 ...
	 */
	template <typename T>
	typename std::enable_if<std::is_integral<T>::value>::type load(T &t, dummy<2> = 0) {
		// get the number of bytes in the stream
		if (signed char size = load_signed_char()) {
			// check for negative value in unsigned type
			if (size < 0 && std::is_unsigned<T>::value) throw portable_archive_exception();
	 ...
	 */
	template <typename T>
	typename std::enable_if<std::is_floating_point<T>::value>::type load(T &t, dummy<3> = 0) {
	...
	template <typename T>
	typename std::enable_if<!std::is_arithmetic<T>::value>::type load(T &t, dummy<4> = 0) {

	...
	*/
	...
	public:
		...
		*/
		...
		template<class T> int32_t push_chunk_multiplexed_noexcept(const T *data_buffer, const double *timestamp_buffer, uint32_t data_buffer_elements, bool pushthrough=true) BOOST_NOEXCEPT {
			try {
		...
		*/
		...
		template<class T> int32_t push_chunk_multiplexed_noexcept(const T *data, uint32_t data_elements, double timestamp=0.0, bool pushthrough=true) BOOST_NOEXCEPT {
			try {

...
using tcp_acceptor_p = std::shared_ptr<tcp::acceptor>;
	...
	*/
	...
	public:
		...
		*/
		tcp_server(const stream_info_impl_p &info, const io_context_p &io,
	...
	private:
		...
		typedef std::shared_ptr<client_session> client_session_p;
		...
		*/
		...
		private:
			...
			std::unique_ptr<class eos::portable_oarchive> outarch_;	// output archive (wrapped around the feed buffer)
			std::istream requeststream_;		// this is a stream on top of the request buffer for convenient parsing			
			char* scratch_{nullptr};			// scratchpad memory (e.g., for endianness conversion)

...
LIBLSL_C_API int32_t lsl_push_sample_rawtpn(lsl_outlet out, void **data,
	...
	try {
		if (outimpl->is_sync_blocking()) {
			...
			if (outimpl->info().channel_format() == cft_string) {
				...
				// TODO: I tried passing void buffer but eventually fail because the convert functions
				//  become ambiguous.
				lsl_error_code_t ec;
				switch(outimpl->info().channel_format()) {
				case cft_int8: ec = outimpl->push_sample_noexcept((const char *)tmp, timestamp, pushthrough);
				case cft_int16: ec = outimpl->push_sample_noexcept((const int16_t *)tmp, timestamp, pushthrough);
				case cft_int32: ec = outimpl->push_sample_noexcept((const int32_t *)tmp, timestamp, pushthrough);
				case cft_int64: ec = outimpl->push_sample_noexcept((const int64_t *)tmp, timestamp, pushthrough);
				case cft_float32: ec = outimpl->push_sample_noexcept((const float *)tmp, timestamp, pushthrough);
				case cft_double64: ec = outimpl->push_sample_noexcept((const double *)tmp, timestamp, pushthrough);

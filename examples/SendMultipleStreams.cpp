...
int main(int argc, char** argv) {
	try {
		...
		for(auto fmt: {lsl::cf_int16, lsl::cf_int32, lsl::cf_int64, lsl::cf_double64, lsl::cf_string})
			outlets.emplace_back(lsl::stream_info(name + std::to_string(fmt), "Example", 1, rate, fmt), 0, 360);
		...
		for (int c = 0; c < rate * 600;) {
			...
			for(auto& outlet: outlets)
				outlet.push_chunk_multiplexed(mychunk, ts);
		}

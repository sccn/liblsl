...
//		elapsed_frames = (device->read_idx - device->write_idx) / device->nchans;
	if (elapsed_frames > 0)
	{
		...
		*buffer = &(device->channel_data[device->write_idx]);
		...
		if ((buf_samples - device->write_idx) < device->nchans)
			device->write_idx = 0;
	}
...
void transmit_thread(void* vargp) {
	...
	lsl_streaminfo info = lsl_create_streaminfo(device->name, "TestLSL",
	 device->nchans, device->srate, cft_int16, device->serial);
	...
	/* make a new outlet */
	lsl_outlet outlet = lsl_create_outlet_d(info, params->chunk_size, params->buffer_dur, params->do_async);

...
{
public:
  work_io_context_runner(lslboost::asio::io_context& io_context)
    : io_context_(io_context) {}
  void operator()() { io_context_.run(); }
private:
  lslboost::asio::io_context& io_context_;
};

resolver_service_base::resolver_service_base(
    lslboost::asio::io_context& io_context)
  : io_context_impl_(lslboost::asio::use_service<io_context_impl>(io_context)),
    work_io_context_(new lslboost::asio::io_context(-1)),
    work_io_context_impl_(lslboost::asio::use_service<
        io_context_impl>(*work_io_context_)),
    work_(lslboost::asio::make_work_guard(*work_io_context_)),
    work_thread_(0)
{
}
    ...
    lslboost::asio::io_context::fork_event fork_ev)
{
  if (work_thread_.get())
  {
    if (fork_ev == lslboost::asio::io_context::fork_prepare)
    {
      work_io_context_->stop();
      work_thread_->join();
    }

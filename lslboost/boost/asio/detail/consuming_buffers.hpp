...
struct prepared_buffers_max<lslboost::array<Elem, N> >
{
  enum { value = N };
};
...
public:
  ...
  explicit consuming_buffers(const Buffers& buffers)
    : buffers_(buffers),
      ...
      next_elem_offset_(0)
  {
    using lslboost::asio::buffer_size;
  ...
  prepared_buffers_type prepare(std::size_t max_size)
  {
    ...
    Buffer_Iterator next = lslboost::asio::buffer_sequence_begin(buffers_);
    Buffer_Iterator end = lslboost::asio::buffer_sequence_end(buffers_);
    ...
    while (next != end && max_size > 0 && (result.count) < result.max_buffers)
    {
      ...
      result.elems[result.count] = lslboost::asio::buffer(next_buf, max_size);
...
public:
  ...
  {
    return lslboost::asio::buffer(buffer_ + total_consumed_, max_size);
  }
...
class consuming_buffers<Buffer, lslboost::array<Elem, 2>,
    typename lslboost::array<Elem, 2>::const_iterator>
{
public:
  // Construct to represent the entire list of buffers.
  explicit consuming_buffers(const lslboost::array<Elem, 2>& buffers)
    : buffers_(buffers),
      total_consumed_(0)
  {
  }
  ...
  // Get the buffer for a single transfer, with a size.
  lslboost::array<Buffer, 2> prepare(std::size_t max_size)
  {
    lslboost::array<Buffer, 2> result = {{
      Buffer(buffers_[0]), Buffer(buffers_[1]) }};
    std::size_t buffer0_size = result[0].size();
    result[0] = lslboost::asio::buffer(result[0] + total_consumed_, max_size);
    result[1] = lslboost::asio::buffer(

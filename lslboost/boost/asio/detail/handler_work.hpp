...
public:
  ...
  {
    execution::execute(
        lslboost::asio::prefer(executor_,
...
public:
  ...
  void dispatch(Function& function, Handler& handler)
  {
    executor_.dispatch(BOOST_ASIO_MOVE_CAST(Function)(function),
        lslboost::asio::get_associated_allocator(handler));
  }
...
public:
  ...
  explicit handler_work_base(const executor_type& ex) BOOST_ASIO_NOEXCEPT
    ...
    : executor_(
        ex.target_type() == typeid(typename IoContext::executor_type)
          ? executor_type()
          : lslboost::asio::prefer(ex, execution::outstanding_work.tracked))
#else // !defined(BOOST_ASIO_NO_TYPEID)
    : executor_(lslboost::asio::prefer(ex, execution::outstanding_work.tracked))
#endif // !defined(BOOST_ASIO_NO_TYPEID)
  {
...
public:
  ...
  handler_work(Handler& handler, const IoExecutor& io_ex) BOOST_ASIO_NOEXCEPT
    : base1_type(io_ex),
      base2_type(lslboost::asio::get_associated_executor(handler, io_ex), io_ex)
  {
  }

...
public:
  ...
  redirect_error_handler(lslboost::system::error_code& ec,
  ...
  typename enable_if<
    !is_same<typename decay<Arg>::type, lslboost::system::error_code>::value
  >::type
    ...
    redirect_error_handler<Handler>* this_handler)
{
  ...
  lslboost_asio_handler_alloc_helpers::allocate(size, this_handler->handler_);
...
struct redirect_error_signature<R(lslboost::system::error_code, Args...)>
{
  typedef R type(Args...);
};

template <typename R, typename... Args>
struct redirect_error_signature<R(const lslboost::system::error_code&, Args...)>
{
  typedef R type(Args...);
};
...
template <typename R>
struct redirect_error_signature<R(lslboost::system::error_code)>
{
  typedef R type();
};

template <typename R>
struct redirect_error_signature<R(const lslboost::system::error_code&)>
{
  typedef R type();
};
...
#define BOOST_ASIO_PRIVATE_REDIRECT_ERROR_DEF(n) \
  ...
  struct redirect_error_signature< \
      R(lslboost::system::error_code, BOOST_ASIO_VARIADIC_TARGS(n))> \
  { \
  ...
  template <typename R, BOOST_ASIO_VARIADIC_TPARAMS(n)> \
  struct redirect_error_signature< \
      R(const lslboost::system::error_code&, BOOST_ASIO_VARIADIC_TARGS(n))> \
  { \
    typedef R type(BOOST_ASIO_VARIADIC_TARGS(n)); \
  }; \
  /**/
...
struct async_result<redirect_error_t<CompletionToken>, Signature>
{
  ...
  struct init_wrapper
  {
    ...
    init_wrapper(lslboost::system::error_code& ec, BOOST_ASIO_MOVE_ARG(Init) init)
      : ec_(ec),

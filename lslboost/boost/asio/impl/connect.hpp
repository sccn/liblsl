...
namespace detail
{
  struct default_connect_condition
  {
    ...
    bool operator()(const lslboost::system::error_code&, const Endpoint&)
    {
      return true;
    }
  };
  ...
  inline typename Protocol::endpoint deref_connect_result(
      Iterator iter, lslboost::system::error_code& ec)
  {
  ...
  inline Iterator call_connect_condition(ConnectCondition& connect_condition,
      const lslboost::system::error_code& ec, Iterator next, Iterator end,
    ...
    void operator()(lslboost::system::error_code ec, int start = 0)
    {
      ...
      switch (start_ = start)
      {
        ...
        for (;;)
        {
          ...
          {
            ec = lslboost::asio::error::not_found;
            lslboost::asio::post(socket_.get_executor(),
                detail::bind_handler(
  ...
  detail::range_connect_op<Protocol BOOST_ASIO_SVC_TARG, EndpointSequence,
    detail::default_connect_condition,
      BOOST_ASIO_HANDLER_TYPE(RangeConnectHandler,
        void (lslboost::system::error_code, typename Protocol::endpoint))>(s,
          endpoints, detail::default_connect_condition(),
            init.completion_handler)(lslboost::system::error_code(), 1);
    ...
    typename enable_if<!is_endpoint_sequence<Iterator>::value>::type*)
{
  ...
  async_completion<IteratorConnectHandler,
    void (lslboost::system::error_code, Iterator)> init(handler);

  detail::iterator_connect_op<Protocol BOOST_ASIO_SVC_TARG, Iterator,
    detail::default_connect_condition, BOOST_ASIO_HANDLER_TYPE(
      IteratorConnectHandler, void (lslboost::system::error_code, Iterator))>(s,

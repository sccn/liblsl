...
public:
   ...
   *
   * For example:
   * @code lslboost::asio::strand<my_executor_type> ex = ...;
   * if (lslboost::asio::query(ex, lslboost::asio::execution::blocking)
   *       == lslboost::asio::execution::blocking.never)
   *   ... @endcode
   */
  ...
  {
    return lslboost::asio::query(executor_, p);
  }
   ...
   *
   * For example:
   * @code lslboost::asio::strand<my_executor_type> ex1 = ...;
      ...
      is_nothrow_require<const Executor&, Property>::value))
  {
    return strand<typename decay<
      typename require_result<const Executor&, Property>::type
        >::type>(lslboost::asio::require(executor_, p), impl_);
  }
   ...
   * For example:
   * @code lslboost::asio::strand<my_executor_type> ex1 = ...;
   * auto ex2 = lslboost::asio::prefer(ex1,
   *     lslboost::asio::execution::blocking.never); @endcode
   */
  template <typename Property>
  typename enable_if<
      ...
      is_nothrow_prefer<const Executor&, Property>::value))
  {
    return strand<typename decay<
      typename prefer_result<const Executor&, Property>::type
        >::type>(lslboost::asio::prefer(executor_, p), impl_);
  }
...
private:
      ...
      >::type* = 0)
  {
    return use_service<detail::strand_executor_service>(
        lslboost::asio::query(ex, execution::context)).create_implementation();
  }
...
/** @defgroup make_strand lslboost::asio::make_strand
 *
 * @brief The lslboost::asio::make_strand function creates a @ref strand object for

...
public:
   ...
   */
  explicit signal_set(lslboost::asio::io_context& io_context)
   ...
   */
  signal_set(lslboost::asio::io_context& io_context, int signal_number_1)
    : basic_io_object<detail::signal_set_service>(io_context)
  {
  ...
  async_wait(BOOST_ASIO_MOVE_ARG(SignalHandler) handler)
  {
    ...
    async_completion<SignalHandler,
      void (lslboost::system::error_code, int)> init(handler);

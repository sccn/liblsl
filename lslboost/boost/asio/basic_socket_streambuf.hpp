...
#if defined(BOOST_ASIO_HAS_BOOST_DATE_TIME) \
  && defined(BOOST_ASIO_USE_BOOST_DATE_TIME_FOR_SOCKET_IOSTREAM)
    typename Clock = lslboost::posix_time::ptime,
...
# endif // !defined(BOOST_ASIO_NO_DEPRECATED)
      ...
      expiry_time_(other.expiry_time_)
  {
    ...
    other.ec_ = lslboost::system::error_code();
...
protected:
  int_type underflow()
  {
    ...
    ec_ = lslboost::asio::error::operation_not_supported;
    ...
    for (;;)
    {
      ...
      {
        ec_ = lslboost::asio::error::timed_out;
        return traits_type::eof();
      }
      ...
      detail::buffer_sequence_adapter<mutable_buffer, mutable_buffer>
        bufs(lslboost::asio::buffer(get_buffer_) + putback_max);
      ...
      if (bytes == 0)
      {
        ec_ = lslboost::asio::error::eof;
        return traits_type::eof();
      }

      // Operation failed.
      if (ec_ != lslboost::asio::error::would_block
          && ec_ != lslboost::asio::error::try_again)
        return traits_type::eof();
  ...
  int_type overflow(int_type c)
  {
    ...
    if (put_buffer_.empty())
    {
      ...
      output_buffer = lslboost::asio::buffer(&ch, sizeof(char_type));
    }
    else
    {
      output_buffer = lslboost::asio::buffer(pbase(),
...
private:
  ...
  void connect_to_endpoints(EndpointIterator begin, EndpointIterator end)
  {
    ...
    ec_ = lslboost::asio::error::not_found;
    for (EndpointIterator i = begin; i != end; ++i)
    {
      ...
      // Operation failed.
      if (ec_ != lslboost::asio::error::in_progress
      ...
      ec_ = lslboost::system::error_code(connect_error,

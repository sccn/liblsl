...
namespace lslboost { namespace optional_detail {
...
template <typename T>
struct optional_value_type< ::lslboost::optional<T> >
{
  typedef T type;
};

}} // namespace lslboost::optional_detail
...
struct is_optional_val_init_candidate<T, U, false>
  : lslboost::conditional< is_convertible_to_T_or_factory<T, U>::value
                      , lslboost::true_type, lslboost::false_type>::type
{};
...
struct optional_uses_direct_storage_for
  : lslboost::conditional<(lslboost::is_scalar<T>::value && !lslboost::is_const<T>::value && !lslboost::is_volatile<T>::value)
                      , lslboost::true_type, lslboost::false_type>::type
{};
...
#  define BOOST_OPTIONAL_BASE_TYPE(T) lslboost::conditional< optional_config::optional_uses_direct_storage_for<T>::value, \
  ...
  : public BOOST_OPTIONAL_BASE_TYPE(T)
{
  ...
  public :
    ...
    // Can throw if T::T(T &&) does
    optional ( rval_reference_type val ) : base(optional_detail::init_value_tag(), lslboost::forward<T>(val))
      {}
#endif
    ...
    /// Creates an optional<T> initialized with 'val' IFF cond is true, otherwise creates an uninitialized optional.
    // Can throw if T::T(T &&) does
    optional ( bool cond, rval_reference_type val ) : base( cond, lslboost::forward<T>(val) )
      {}
#endif
    ...
    explicit optional ( optional<U> const& rhs
                        ...
                        ,BOOST_DEDUCED_TYPENAME lslboost::enable_if< optional_detail::is_optional_constructible<T, U const&>, bool>::type = true
#endif
                      )
      :
      base()
...
#ifndef BOOST_OPTIONAL_DETAIL_NO_SFINAE_FRIENDLY_CONSTRUCTORS
                        ,BOOST_DEDUCED_TYPENAME lslboost::enable_if< optional_detail::is_optional_constructible<T, U>, bool>::type = true
#endif
                      )
      :
      base()
  ...
  explicit optional ( Expr&& expr,
                      BOOST_DEDUCED_TYPENAME lslboost::enable_if< optional_detail::is_optional_val_init_candidate<T, Expr>, bool>::type = true
  )
    ...
    template <typename F>
    optional<typename lslboost::result_of<F(reference_type)>::type> map(F f) &
      {
        if (this->has_value())
          return f(get());
        else
    ...
    template <typename F>
    optional<typename lslboost::result_of<F(reference_const_type)>::type> map(F f) const&
      {
        if (this->has_value())
          return f(get());
        else
    ...
    template <typename F>
    optional<typename lslboost::result_of<F(reference_type_of_temporary_wrapper)>::type> map(F f) &&
      {
        if (this->has_value())
          return f(lslboost::move(this->get()));
        else
          return none;
      }

    template <typename F>
    optional<typename optional_detail::optional_value_type<typename lslboost::result_of<F(reference_type)>::type>::type> flat_map(F f) &
      {
        if (this->has_value())
          return f(get());
        else
    ...
    template <typename F>
    optional<typename optional_detail::optional_value_type<typename lslboost::result_of<F(reference_const_type)>::type>::type> flat_map(F f) const&
      {
        if (this->has_value())
          return f(get());
        else
    ...
    template <typename F>
    optional<typename optional_detail::optional_value_type<typename lslboost::result_of<F(reference_type_of_temporary_wrapper)>::type>::type> flat_map(F f) &&
      {
        if (this->has_value())
          return f(lslboost::move(get()));
        else
          return none;
      }
...
template<class T>
inline
optional<BOOST_DEDUCED_TYPENAME lslboost::decay<T>::type> make_optional ( T && v  )
{
  return optional<BOOST_DEDUCED_TYPENAME lslboost::decay<T>::type>(lslboost::forward<T>(v));
}

// Returns optional<T>(cond,v)
template<class T>
inline
optional<BOOST_DEDUCED_TYPENAME lslboost::decay<T>::type> make_optional ( bool cond, T && v )
{
  return optional<BOOST_DEDUCED_TYPENAME lslboost::decay<T>::type>(cond,lslboost::forward<T>(v));
}

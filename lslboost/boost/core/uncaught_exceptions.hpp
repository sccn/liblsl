...
inline unsigned int uncaught_exceptions() BOOST_NOEXCEPT
{
    ...
    unsigned int count;
    std::memcpy(&count, static_cast< const unsigned char* >(lslboost::core::detail::_getptd()) + (sizeof(void*) == 8u ? 0x100 : 0x90), sizeof(count)); // _tiddata::_ProcessingThrow, x32 offset - 0x90, x64 - 0x100
    return count;
#else

...
{
    lslboost::allocator_destroy(a, p);
}
...
{
    while (n > 0) {
        lslboost::allocator_destroy(a, p + --n);
    }
}
...
public:
    ...
    ~alloc_destroyer() {
        lslboost::alloc_destroy_n(a_, p_, n_);
    }
...
{
    lslboost::allocator_construct(a, p);
}
...
{
    lslboost::allocator_construct(a, p, std::forward<U>(u),
        std::forward<V>(v)...);
}
#else
...
{
    lslboost::allocator_construct(a, p, u);
}
...
alloc_construct_n(A& a, T* p, std::size_t n)
{
    ...
    for (std::size_t& i = hold.size(); i < n; ++i) {
        lslboost::allocator_construct(a, p + i);
    }
    hold.size() = 0;
}
...
alloc_construct_n(A& a, T* p, std::size_t n, const T* l, std::size_t m)
{
    detail::alloc_destroyer<A, T> hold(a, p);
    for (std::size_t& i = hold.size(); i < n; ++i) {
        lslboost::allocator_construct(a, p + i, l[i % m]);
    }
    hold.size() = 0;
}
...
alloc_construct_n(A& a, T* p, std::size_t n, I b)
{
    ...
    for (std::size_t& i = hold.size(); i < n; void(++i), void(++b)) {
        lslboost::allocator_construct(a, p + i, *b);
    }
    hold.size() = 0;
}

...
-> decltype(lslboost::pointer_traits<T>::to_address(v))
{
    return lslboost::pointer_traits<T>::to_address(v);
}
...
{
    return lslboost::detail::ptr_address(v.operator->(), 0);
}
...
{
    return lslboost::detail::ptr_address(v, 0);
}
#else
template<class T>
...
to_address(const T& v) BOOST_NOEXCEPT
{
    return lslboost::to_address(v.operator->());
}
#endif

                                ...
                                std::size_t offset, unsigned char exceptions)
    {
      ...
      // argument type modifiers
        while (start != last) {
            switch (wrap_narrow(fac, *start, 0)) {
                ...
                case 'z':
                case 'L':
                    // lslboost::format ignores argument type modifiers as it relies on
                    // the type of the argument fed into it by operator %
                    break;

                // Note that the ptrdiff_t argument type 't' from C++11 is not honored
                // because it was already in use as the tabulation specifier in lslboost::format
                // case 't':
      ...
      parse_conversion_specification:
        ...
        switch (wrap_narrow(fac, *start, 0))
        {
            ...
            // Tabulation (a lslboost::format extension)
            case 'T':
                ++start;
                if( start >= last) {
                    maybe_throw_exception(exceptions, start-start0+offset, fstring_size);
            ...
            // %n is insecure and ignored by lslboost::format
            case 'n' :

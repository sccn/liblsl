...
{
   lslboost::uint64_t low_mask;
   std::set<std::size_t> mask_set;
public:
   indexed_bit_flag() : low_mask(0) {}
   void set(std::size_t i)
   {
      if (i < std::numeric_limits<lslboost::uint64_t>::digits - 1)
         low_mask |= static_cast<lslboost::uint64_t>(1u) << i;
      else
         mask_set.insert(i);
   }
   ...
   {
      if (i < std::numeric_limits<lslboost::uint64_t>::digits - 1)
         return low_mask & static_cast<lslboost::uint64_t>(1u) << i ? true : false;
      else

...
lslboost::intmax_t global_toi(const charT*& p1, const charT* p2, int radix, const traits& t)
{
   ...
   lslboost::intmax_t limit = (std::numeric_limits<lslboost::intmax_t>::max)() / radix;
...
template <class charT>
inline typename lslboost::enable_if_c<(sizeof(charT) > 1), const charT*>::type get_escape_R_string()
{
#ifdef BOOST_MSVC
#  pragma warning(push)
...
template <class charT>
inline typename lslboost::disable_if_c<(sizeof(charT) > 1), const charT*>::type get_escape_R_string()
{
#ifdef BOOST_MSVC
#  pragma warning(push)

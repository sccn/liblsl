   ...
   , Compare comp)
{
   ...
   if (first1 != last1){
      ...
      ::new((iterator_to_raw_pointer)(last2), boost_move_new_t()) value_type(::lslboost::move(*first1));
      ...
      for (++last2; ++first1 != last1; ++last2){
         ...
         if (comp(*first1, *--k2)){
            ::new((iterator_to_raw_pointer)(j2), boost_move_new_t()) value_type(::lslboost::move(*k2));
            d.incr();
            for (--j2; k2 != first2 && comp(*first1, *--k2); --j2)
               *j2 = ::lslboost::move(*k2);
            *j2 = ::lslboost::move(*first1);
         }
         else{
            ::new((iterator_to_raw_pointer)(j2), boost_move_new_t()) value_type(::lslboost::move(*first1));
            d.incr();
         }
      }
      d.release();

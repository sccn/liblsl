...
namespace pdqsort_detail {
   ...
   struct pair
   {
    ...
    inline bool partial_insertion_sort(Iter begin, Iter end, Compare comp) {
        typedef typename lslboost::movelib::iterator_traits<Iter>::value_type T;
        typedef typename lslboost::movelib::iterator_traits<Iter>::size_type  size_type;
        ...
        for (Iter cur = begin + 1; cur != end; ++cur) {
            ...
            if (comp(*sift, *sift_1)) {
                T tmp = lslboost::move(*sift);

                do { *sift-- = lslboost::move(*sift_1); }
                while (sift != begin && comp(tmp, *--sift_1));

                *sift = lslboost::move(tmp);
    ...
    inline void sort2(Iter a, Iter b, Compare comp) {
        if (comp(*b, *a)) lslboost::adl_move_iter_swap(a, b);
    }
    ...
    pdqsort_detail::pair<Iter, bool> partition_right(Iter begin, Iter end, Compare comp) {
        ...
        // above.
        while (first < last) {
            lslboost::adl_move_iter_swap(first, last);
            while (comp(*++first, pivot));
        ...
        *begin = lslboost::move(*pivot_pos);
        *pivot_pos = lslboost::move(pivot);
   ...
   void pdqsort_loop( Iter begin, Iter end, Compare comp
                    , typename lslboost::movelib::iterator_traits<Iter>::size_type bad_allowed
                    , bool leftmost = true)
   {
        typedef typename lslboost::movelib::iterator_traits<Iter>::size_type size_type;
        ...
        while (true) {
            ...
            if (size > ninther_threshold) {
                ...
                lslboost::adl_move_iter_swap(begin, begin + s2);
            ...
            if (highly_unbalanced) {
                ...
                if (--bad_allowed == 0) {
                    lslboost::movelib::heap_sort(begin, end, comp);
                    return;
                }

                if (l_size >= insertion_sort_threshold) {
                    lslboost::adl_move_iter_swap(begin,             begin + l_size / 4);
                    lslboost::adl_move_iter_swap(pivot_pos - 1, pivot_pos - l_size / 4);

                    if (l_size > ninther_threshold) {
                        lslboost::adl_move_iter_swap(begin + 1,         begin + (l_size / 4 + 1));
                        lslboost::adl_move_iter_swap(begin + 2,         begin + (l_size / 4 + 2));
                        lslboost::adl_move_iter_swap(pivot_pos - 2, pivot_pos - (l_size / 4 + 1));
                        lslboost::adl_move_iter_swap(pivot_pos - 3, pivot_pos - (l_size / 4 + 2));
                    }
                }
                
                if (r_size >= insertion_sort_threshold) {
                    lslboost::adl_move_iter_swap(pivot_pos + 1, pivot_pos + (1 + r_size / 4));
                    lslboost::adl_move_iter_swap(end - 1,                   end - r_size / 4);
                    
                    if (r_size > ninther_threshold) {
                        lslboost::adl_move_iter_swap(pivot_pos + 2, pivot_pos + (2 + r_size / 4));
                        lslboost::adl_move_iter_swap(pivot_pos + 3, pivot_pos + (3 + r_size / 4));
                        lslboost::adl_move_iter_swap(end - 2,             end - (1 + r_size / 4));
                        lslboost::adl_move_iter_swap(end - 3,             end - (2 + r_size / 4));
                    }
                }
            } else {

      ...
      typename iterator_traits<BidirectionalIterator1>::size_type buffer_size)
{
   if (len1 > len2 && len2 <= buffer_size)
   {
      ...
      {
         BidirectionalIterator2 buffer_end = lslboost::move(middle, last, buffer);
         lslboost::move_backward(first, middle, last);
         return lslboost::move(buffer, buffer_end, first);
      }
      else
         return first;
   }
   else if (len1 <= buffer_size)
   {
      ...
      {
         BidirectionalIterator2 buffer_end = lslboost::move(first, middle, buffer);
         BidirectionalIterator1 ret = lslboost::move(middle, last, first);
         lslboost::move(buffer, buffer_end, ret);
         return ret;
      }
      else
         return last;
                           ...
                           typename iterator_traits<BidirectionalIterator>::size_type uninitialized_len)
{
   ...
   if(uninitialized_len)
   {
      ...
      ::lslboost::movelib::adaptive_xbuf<value_type, RandRawIt> xbuf(uninitialized, uninitialized_len);

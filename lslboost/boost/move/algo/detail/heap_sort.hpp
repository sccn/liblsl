...
{
   typedef typename lslboost::movelib::iterator_traits<RandomAccessIterator>::size_type  size_type;
   typedef typename lslboost::movelib::iterator_traits<RandomAccessIterator>::value_type value_type;
   ...
   static void adjust_heap(RandomAccessIterator first, size_type hole_index, size_type const len, value_type &value, Compare comp)
   {
      ...
      while (second_child < len) {
         if (comp(*(first + second_child), *(first + (second_child - 1))))
            second_child--;
         *(first + hole_index) = lslboost::move(*(first + second_child));
         hole_index = second_child;
         second_child = 2 * (second_child + 1);
      }
      if (second_child == len) {
         *(first + hole_index) = lslboost::move(*(first + (second_child - 1)));
      ...
      {  //push_heap-like ending
         ...
         while (hole_index > top_index && comp(*(first + parent), value)) {
            *(first + hole_index) = lslboost::move(*(first + parent));
            hole_index = parent;
            parent = (hole_index - 1) / 2;
         }    
         *(first + hole_index) = lslboost::move(value);
      }
   }
   ...
   static void make_heap(RandomAccessIterator first, RandomAccessIterator last, Compare comp)
   {
      ...
      if (len > 1) {
         ...
         do {
            value_type v(lslboost::move(*(first + parent)));
   ...
   static void sort_heap(RandomAccessIterator first, RandomAccessIterator last, Compare comp)
   {
      ...
      while (len > 1) {
         ...
         --last;
         value_type v(lslboost::move(*last));
         *last = lslboost::move(*first);
   ...
   static void sort(RandomAccessIterator first, RandomAccessIterator last, Compare comp)
   {
      ...
      BOOST_ASSERT(lslboost::movelib::is_sorted(first, last, comp));
   }
};

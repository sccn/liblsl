   ...
   , Compare comp)
{
   ...
   ::lslboost::ignore_unused(key_count);
   //BOOST_ASSERT(n_block_a || n_block_b);
   BOOST_MOVE_ADAPTIVE_SORT_INVARIANT(lslboost::movelib::is_sorted_and_unique(key_first, key_first + key_count, key_comp));
   ...
   {  //Selection sort blocks
      ...
      for (RandIt f = first+l_irreg1; n_block_left; --n_block_left, ++key_range2, f += l_block, min_check -= min_check != 0, max_check -= max_check != 0) {
         ...
         BOOST_MOVE_ADAPTIVE_SORT_INVARIANT(lslboost::movelib::is_sorted(f, f+l_block, comp));
         BOOST_MOVE_ADAPTIVE_SORT_INVARIANT(lslboost::movelib::is_sorted(first_min, first_min + l_block, comp));
         BOOST_MOVE_ADAPTIVE_SORT_INVARIANT((f == (first+l_irreg1)) || !comp(*f, *(f-l_block)));
      }
   }
   BOOST_MOVE_ADAPTIVE_SORT_INVARIANT(lslboost::movelib::is_sorted(first+l_irreg1+n_bef_irreg2*l_block, first_irr2, comp));
   ...
   for(RandItKeys key_next = key_first; key_next != key_end; ++key_next){
      ...
      last1 += l_block;
      BOOST_MOVE_ADAPTIVE_SORT_INVARIANT(lslboost::movelib::is_sorted(first, first1, comp));
   }

   merge_bufferless(is_range1_A ? first1 : last1, first_irr2, last_irr2, comp);
   BOOST_MOVE_ADAPTIVE_SORT_INVARIANT(lslboost::movelib::is_sorted(first, last_irr2, comp));
}
      ...
      , XBuf &xbuf)
{
   ...
   BOOST_MOVE_ADAPTIVE_SORT_INVARIANT(lslboost::movelib::is_sorted(first, last, lslboost::movelib::unantistable(comp)));
}
                    ...
                    , XBuf & xbuf)
{
   ...
   BOOST_ASSERT(lslboost::movelib::is_sorted_and_unique(first, last, comp));
}
   ...
   , Compare comp, bool const is_stable, Op op)
{
   ...
   for(; n_block_left; --n_block_left, ++key_first, min_check -= min_check != 0, max_check -= max_check != 0){
      ...
      lslboost::ignore_unused(last_min);

      BOOST_MOVE_ADAPTIVE_SORT_INVARIANT(lslboost::movelib::is_sorted(first_reg, last_reg, comp));
      BOOST_MOVE_ADAPTIVE_SORT_INVARIANT(!next_key_idx || lslboost::movelib::is_sorted(first_min, last_min, comp));
      BOOST_MOVE_ADAPTIVE_SORT_INVARIANT((!next_key_idx || !comp(*first_reg, *first_min )));

      OutputIt orig_dest = dest;
      lslboost::ignore_unused(orig_dest);
      ...
      BOOST_MOVE_ADAPTIVE_SORT_INVARIANT(lslboost::movelib::is_sorted(orig_dest, dest, comp));
...
//   BOOST_ASSERT(n_block_a || n_block_b);
   ...
   for (; n_block_left; --n_block_left, ++key_range2, min_check -= min_check != 0, max_check -= max_check != 0) {
      ...
      BOOST_MOVE_ADAPTIVE_SORT_INVARIANT(lslboost::movelib::is_sorted(first1, last1, comp));
      BOOST_MOVE_ADAPTIVE_SORT_INVARIANT(lslboost::movelib::is_sorted(first2, last2, comp));
      BOOST_MOVE_ADAPTIVE_SORT_INVARIANT(!n_block_left || lslboost::movelib::is_sorted(first_min, last_min, comp));
      ...
      if(is_range1_A == is_range2_A){
         ...
         lslboost::ignore_unused(unmerged);
         BOOST_MOVE_ADAPTIVE_SORT_INVARIANT(lslboost::movelib::is_sorted(first-l_block, unmerged, comp));
   ...
   BOOST_MOVE_ADAPTIVE_SORT_INVARIANT(lslboost::movelib::is_sorted(first-l_block, buffer, comp));
   ...
   , RandItBuf const buf_first)
{
   ...
   for (; n_block_left; --n_block_left, ++key_range2, min_check -= min_check != 0, max_check -= max_check != 0) {
      ...
      lslboost::ignore_unused(buffer_empty);
      BOOST_MOVE_ADAPTIVE_SORT_INVARIANT(buffer_empty ? lslboost::movelib::is_sorted(first1, last1, comp) : lslboost::movelib::is_sorted(buffer, buffer_end, comp));
      BOOST_MOVE_ADAPTIVE_SORT_INVARIANT(lslboost::movelib::is_sorted(first2, last2, comp));
      BOOST_MOVE_ADAPTIVE_SORT_INVARIANT(!n_block_left || lslboost::movelib::is_sorted(first_min, last_min, comp));
      ...
      if(is_range1_A == is_range2_A){
         ...
         BOOST_MOVE_ADAPTIVE_SORT_INVARIANT(lslboost::movelib::is_sorted(first2, last2, comp));
         BOOST_MOVE_ADAPTIVE_SORT_INVARIANT(lslboost::movelib::is_sorted(first_min, last_min, comp));
         first1 = first2;
         BOOST_MOVE_ADAPTIVE_SORT_INVARIANT(lslboost::movelib::is_sorted(first, first1, comp));
      }
      else {
         ...
         first1 = unmerged;
         BOOST_MOVE_ADAPTIVE_SORT_INVARIANT(lslboost::movelib::is_sorted(first, unmerged, comp));
      }
   ...
   RandIt res = op(forward_t(), buffer, buffer_end, first1);
   lslboost::ignore_unused(res);
   BOOST_MOVE_ADAPTIVE_SORT_INVARIANT(lslboost::movelib::is_sorted(first, res, comp));
   BOOST_MOVE_ADAPTIVE_SORT_PRINT_L2("   merge_blocks_w_fwd: ", len);
   ...
   BOOST_MOVE_ADAPTIVE_SORT_INVARIANT(lslboost::movelib::is_sorted(dest, last_irr2, comp));
      ...
      , Op op)
{
   ...
   for(; l_merged < l_build_buf && l_left_space >= l_merged; l_merged*=2){
      ...
      while((elements_in_blocks - p0) > 2*l_merged) {
         ...
         BOOST_MOVE_ADAPTIVE_SORT_INVARIANT(lslboost::movelib::is_sorted(pos-l_merged, pos+l_merged, comp));
         p0 += 2*l_merged;
         pos = first_block+p0;
      }
      if((elements_in_blocks-p0) > l_merged) {
         op_merge_left(pos-l_merged, pos, pos+l_merged, first_block+elements_in_blocks, comp, op);
         BOOST_MOVE_ADAPTIVE_SORT_INVARIANT(lslboost::movelib::is_sorted(pos-l_merged, pos-l_merged+(first_block+elements_in_blocks-pos), comp));
      }
      else {
         op(forward_t(), pos, first_block+elements_in_blocks, pos-l_merged);
         BOOST_MOVE_ADAPTIVE_SORT_INVARIANT(lslboost::movelib::is_sorted(pos-l_merged, first_block+elements_in_blocks-l_merged, comp));
      }

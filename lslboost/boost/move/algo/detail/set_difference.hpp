   ...
   (InputIt1 first1, InputIt1 last1, InputIt2 first2, InputIt2 last2, OutputIt result, Compare comp)
{
   while (first1 != last1) {
      if (first2 == last2)
         return lslboost::move_detail::copy(first1, last1, result);
   ...
   while (first1 != last1) {
      ...
      if (first2 == last2){
         ...
         if (!comp(*first2, *first1)) {
            ...
            return lslboost::movelib::set_difference
               ( lslboost::make_move_iterator(++first1)
               , lslboost::make_move_iterator(last1)
   ...
   while (first1 != last1) {
      ...
      if (first2 == last2){
         ...
         if (++first1 != last1 && !comp(*result, *first1)) {
            ...
            return lslboost::movelib::set_unique_difference
               ( lslboost::make_move_iterator(first1)
               , lslboost::make_move_iterator(last1)
               , first2, last2, ++result, comp);
         }

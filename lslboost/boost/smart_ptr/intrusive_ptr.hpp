...
public:
    ...
    template<class U>
    BOOST_SP_CXX20_CONSTEXPR intrusive_ptr( intrusive_ptr<U> const & rhs, typename lslboost::detail::sp_enable_if_convertible<U,T>::type = lslboost::detail::sp_empty() )
    : px( rhs.get() )
    {
...
// Move support
    ...
    template<class U>
    BOOST_SP_CXX20_CONSTEXPR intrusive_ptr(intrusive_ptr<U> && rhs, typename lslboost::detail::sp_enable_if_convertible<U,T>::type = lslboost::detail::sp_empty())
    : px( rhs.px )
    {
        rhs.px = 0;
    }
...
template< class T > std::size_t hash_value( lslboost::intrusive_ptr<T> const & p ) noexcept
{
...
template<class T> struct hash< ::lslboost::intrusive_ptr<T> >
{
    std::size_t operator()( ::lslboost::intrusive_ptr<T> const & p ) const noexcept
    {

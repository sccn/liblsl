...
//
namespace lslboost{ namespace type_traits_detail{
...
{
  template<typename Q> static lslboost::true_type  test(decltype(&Q::operator())*);
  template<typename>   static lslboost::false_type test(...);
...
struct is_likely_stateless_lambda<
  T,
  typename lslboost::enable_if_c<has_one_operator_call<T>::value>::type> : 
     lslboost::is_convertible<T, typename equivalent_function_pointer<T>::type
>{};
...
namespace lslboost {
   namespace type_traits_detail {
      ...
      template<typename T>
      struct is_likely_stateless_lambda : public lslboost::integral_constant<bool, false> {};
}}

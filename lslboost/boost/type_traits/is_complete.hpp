...
#define BOOST_TT_HAS_WORKING_IS_COMPLETE
   ...
   template <class T> struct is_complete
      : public integral_constant<bool, ::lslboost::is_function<typename lslboost::remove_reference<T>::type>::value || (sizeof(lslboost::detail::check_is_complete<T>(0)) != sizeof(char))> {};
   ...
   namespace detail
   {
      ...
      {
         template <class U, class = decltype(sizeof(lslboost::declval< U >())) >
   ...
   template <class T>
   struct is_complete : lslboost::integral_constant<bool, ::lslboost::is_function<typename lslboost::remove_reference<T>::type>::value || ::lslboost::detail::is_complete_imp<T>::value>
   {};
   template <class T>
   struct is_complete<T&> : lslboost::is_complete<T> {};

...
namespace lslboost{
   ...
   {
      BOOST_STATIC_ASSERT_MSG(::lslboost::is_complete<T>::value, "The target type must be complete in order to test for constructibility");
   };
   template <class T, class Arg> struct is_constructible<T, Arg> : public integral_constant<bool, is_destructible<T>::value && sizeof(lslboost::detail::is_constructible_imp::test1<T, Arg>(0)) == sizeof(lslboost::type_traits::yes_type)>
   {
      BOOST_STATIC_ASSERT_MSG(::lslboost::is_complete<T>::value, "The target type must be complete in order to test for constructibility");
   };

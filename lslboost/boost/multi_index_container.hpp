...
BOOST_MULTI_INDEX_PROTECTED_IF_MEMBER_TEMPLATE_FRIENDS:
  ...
  std::pair<node_type*,bool> insert_ref_(T& t)
  {
    ...
    BOOST_TRY{
      new(lslboost::addressof(x->value())) value_type(t);
      BOOST_TRY{
        ...
        if(res==x){
          ...
          lslboost::detail::allocator::destroy(lslboost::addressof(x->value()));
          deallocate_node(x);
          return std::pair<node_type*,bool>(res,false);
        }
      }
    ...
    BOOST_MULTI_INDEX_FUNCTION_PARAM_PACK)
  {
    ...
    BOOST_TRY{
      detail::vartempl_placement_new(
        lslboost::addressof(x->value()),BOOST_MULTI_INDEX_FORWARD_PARAM_PACK);
      BOOST_TRY{
        node_type* res=super::insert_(x->value(),x,detail::emplaced_tag());
        if(res==x){
          ++node_count;
  ...
  void save(Archive& ar,const unsigned int version)const
  {
    ...
    for(iterator it=super::begin(),it_end=super::end();it!=it_end;++it){
      serialization::save_construct_data_adl(
        ar,lslboost::addressof(*it),value_version);
  ...
  void load(Archive& ar,const unsigned int version)
  {
    ...
    for(std::size_t n=0;n<s;++n){
      ...
      ar.reset_object_address(
        lslboost::addressof(p.first->value()),lslboost::addressof(value.get()));

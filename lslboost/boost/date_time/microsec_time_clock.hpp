...
namespace date_time {
  ...
  private:
    static time_type create_time(time_converter converter)
    {
      ...
      lslboost::winapi::GetSystemTimeAsFileTime(&ft);
#if BOOST_WORKAROUND(__MWERKS__, BOOST_TESTED_AT(0x3205))
      // Some runtime library implementations expect local times as the norm for ctime functions.
      {
        lslboost::winapi::FILETIME_ local_ft;
        lslboost::winapi::FileTimeToLocalFileTime(&ft, &local_ft);
        ft = local_ft;
      }
#endif

      lslboost::uint64_t micros = file_time_to_microseconds(ft); // it will not wrap, since ft is the current time
...
#error Internal Boost.DateTime error: BOOST_DATE_TIME_HAS_HIGH_PRECISION_CLOCK is defined, however neither gettimeofday nor FILETIME support is detected.
     ...
     */
    static lslboost::uint64_t file_time_to_microseconds(lslboost::winapi::FILETIME_ const& ft)
    {
      // shift is difference between 1970-Jan-01 & 1601-Jan-01
      // in 100-nanosecond units
      const lslboost::uint64_t shift = 116444736000000000ULL; // (27111902 << 32) + 3577643008

      // 100-nanos since 1601-Jan-01
      lslboost::uint64_t ft_as_integer = (static_cast< lslboost::uint64_t >(ft.dwHighDateTime) << 32) | static_cast< lslboost::uint64_t >(ft.dwLowDateTime);

...
public:
    ...
    BOOST_STATIC_ASSERT_MSG(sizeof(value_type) > 0u, "lslboost::atomic<T> requires T to be a complete type");
#if !defined(BOOST_ATOMIC_DETAIL_NO_CXX11_IS_TRIVIALLY_COPYABLE)
    BOOST_STATIC_ASSERT_MSG(atomics::detail::is_trivially_copyable< value_type >::value, "lslboost::atomic<T> requires T to be a trivially copyable type");
#endif
...
typedef atomic< lslboost::intptr_t > atomic_intptr_t;
typedef atomic< lslboost::uintptr_t > atomic_uintptr_t;
#endif

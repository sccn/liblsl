...
public:
    ...
    BOOST_STATIC_ASSERT_MSG(sizeof(value_type) > 0u, "lslboost::ipc_atomic_ref<T> requires T to be a complete type");
#if !defined(BOOST_ATOMIC_DETAIL_NO_CXX11_IS_TRIVIALLY_COPYABLE)
    BOOST_STATIC_ASSERT_MSG(atomics::detail::is_trivially_copyable< value_type >::value, "lslboost::ipc_atomic_ref<T> requires T to be a trivially copyable type");
#endif

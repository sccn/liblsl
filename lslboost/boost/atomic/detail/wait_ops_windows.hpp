...
typedef lslboost::winapi::BOOL_ BOOST_WINAPI_WINAPI_CC
wait_on_address_t(
    volatile lslboost::winapi::VOID_* addr,
    lslboost::winapi::PVOID_ compare_addr,
    lslboost::winapi::SIZE_T_ size,
    lslboost::winapi::DWORD_ timeout_ms);

typedef lslboost::winapi::VOID_ BOOST_WINAPI_WINAPI_CC
wake_by_address_t(lslboost::winapi::PVOID_ addr);
...
BOOST_FORCEINLINE void ensure_wait_functions_initialized() BOOST_NOEXCEPT
{
    ...
    if (BOOST_LIKELY(once_flag_operations::load(wait_functions_once_flag.m_flag, lslboost::memory_order_acquire) == 0u))
        return;
    ...
    public atomics::detail::wait_operations_generic< Base, false >
{
    ...
    static BOOST_FORCEINLINE storage_type wait(storage_type const volatile& storage, storage_type old_val, memory_order order) BOOST_NOEXCEPT
    {
        ...
        if (BOOST_LIKELY(atomics::detail::wait_on_address != NULL))
        {
            ...
            {
                atomics::detail::wait_on_address(const_cast< storage_type* >(&storage), &old_val, Size, lslboost::winapi::infinite);

...
private:
    ...
    {
        storage_type old_value = core_operations::load(this->storage(), lslboost::memory_order_relaxed);
        while (true)
        {
            ...
            bool res = core_operations::compare_exchange_weak(this->storage(), old_value, new_value, lslboost::memory_order_relaxed, lslboost::memory_order_relaxed);
            if (BOOST_LIKELY(res))
                break;
        }
    }

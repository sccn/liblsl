...
namespace lslboost { namespace parameter { namespace aux {
    ...
    template <typename U>
    ::lslboost::parameter::aux::yes_tag
        is_cv_reference_wrapper_check(
            ::lslboost::reference_wrapper<U> const volatile*
        );

    ::lslboost::parameter::aux::no_tag is_cv_reference_wrapper_check(...);
...
namespace lslboost { namespace parameter { namespace aux {
    ...
    template <typename T>
    using is_cv_reference_wrapper = ::lslboost::mp11::mp_bool<
        sizeof(
            ::lslboost::parameter::aux::is_cv_reference_wrapper_check(
                static_cast<
    ...
    // tagged_argument<>.  Can't use lslboost::unwrap_reference<> here
    // because it doesn't handle the case where T = reference_wrapper<U> cv.
    template <typename T>
    using unwrap_cv_reference = ::lslboost::mp11::mp_eval_if<
        ::lslboost::parameter::aux::is_cv_reference_wrapper<T>
      , ::lslboost::parameter::aux::unwrap_cv_reference_impl<T>
    ...
    >
    struct unwrap_cv_reference : ::lslboost::remove_reference<T>
    {
    };

    template <typename T>
    struct unwrap_cv_reference<T const,::lslboost::mpl::false_>
    {
        typedef T const type;
    };

    template <typename T>
    struct unwrap_cv_reference<T,::lslboost::mpl::true_> : T
    {
    };
    ...
    template <typename T>
    struct unwrap_cv_reference_impl : ::lslboost::remove_reference<T>::type
    {
    };

   ...
   : public Allocator
{
   ...
   typedef lslboost::container::allocator_traits<allocator_type>  allocator_traits_type;
   ...
   explicit vector_alloc_holder(BOOST_FWD_REF(AllocConvertible) a) BOOST_NOEXCEPT_OR_NOTHROW
      : allocator_type(lslboost::forward<AllocConvertible>(a)), m_start(), m_size(), m_capacity()
   {}
   ...
   vector_alloc_holder(vector_uninitialized_size_t, BOOST_FWD_REF(AllocConvertible) a, SizeType initial_size)
      : allocator_type(lslboost::forward<AllocConvertible>(a))
      , m_start()
      ...
      , m_capacity()
   {
      if (initial_size > size_type(-1)){
         lslboost::container::throw_length_error("get_next_capacity, allocator's max size reached");
      }
   ...
   vector_alloc_holder(initial_capacity_t, pointer p, size_type n, BOOST_FWD_REF(AllocFwd) a)
      : allocator_type(::lslboost::forward<AllocFwd>(a))
      , m_start(p)
      , m_size()
      , m_capacity(n)
   {}
   ...
   : public Allocator
{
   ...
   template<class AllocConvertible>
   explicit vector_alloc_holder(BOOST_FWD_REF(AllocConvertible) a) BOOST_NOEXCEPT_OR_NOTHROW
      : allocator_type(lslboost::forward<AllocConvertible>(a)), m_size()
   {}
      ...
      , m_size(holder.m_size) //Size is initialized here so vector should only call uninitialized_xxx after this
   {
      ...
      ::lslboost::container::destroy_alloc_n
         (this->alloc(), lslboost::movelib::to_raw_pointer(holder.start()), m_size);
      holder.m_size = 0;
   }
...
//! \tparam Options A type produced from \c lslboost::container::vector_options.
...
public:
   ...
   typedef ::lslboost::container::allocator_traits<allocator_type>                        allocator_traits_t;
...
private:
   ...
   typedef typename dtl::version<allocator_type>::type alloc_version;
   typedef lslboost::container::vector_alloc_holder
   ...
   BOOST_CONTAINER_FORCEINLINE vector(initial_capacity_t, pointer initial_memory, size_type capacity, BOOST_FWD_REF(AllocFwd) a)
      : m_holder(initial_capacity_t(), initial_memory, capacity, ::lslboost::forward<AllocFwd>(a))
   {}
...
//           ) -> vector<typename iterator_traits<InIt>::value_type, new_allocator<typename iterator_traits<InIt>::value_type>>;
      ...
      )
   {
      ...
      const typename iterator_traits<FwdIt>::size_type sz = lslboost::container::iterator_distance(first, last);
      if (sz > size_type(-1)){
         lslboost::container::throw_length_error("vector::assign, FwdIt's max length reached");
      }
   ...
   BOOST_CONTAINER_FORCEINLINE reference emplace_back(BOOST_FWD_REF(Args)...args)
   {
      ...
      if (BOOST_LIKELY(this->room_enough())){
         ...
         return *this->priv_insert_forward_range_no_capacity
            (p, 1, proxy_t(::lslboost::forward<Args>(args)...), alloc_version());
      }
   }
   ...
   BOOST_CONTAINER_FORCEINLINE iterator emplace(const_iterator position, BOOST_FWD_REF(Args) ...args)
   {
      ...
      return this->priv_insert_forward_range( vector_iterator_get_ptr(position), 1
                                            , proxy_t(::lslboost::forward<Args>(args)...));
   }
      ...
      )
   {
      ...
      const typename iterator_traits<FwdIt>::size_type sz = lslboost::container::iterator_distance(first, last);
      if (sz > size_type(-1)){
         lslboost::container::throw_length_error("vector::insert, FwdIt's max length reached");
      }
   ...
   iterator erase(const_iterator position)
   {
      ...
      (void)::lslboost::container::move(pos_ptr + 1, end_ptr, pos_ptr);
   ...
   iterator erase(const_iterator first, const_iterator last)
   {
      ...
      if(first != last){
         ...
         T* const new_last_ptr = lslboost::movelib::to_raw_pointer(lslboost::container::move(last_ptr, old_end_ptr, first_ptr));
         ...
         if(!value_traits::trivial_dctr_after_move || old_end_ptr == last_ptr){
            lslboost::container::destroy_alloc_n(this->get_stored_allocator(), new_last_ptr, n);
         }
   ...
   BOOST_CONTAINER_FORCEINLINE void merge(InputIt first, InputIt last, Compare comp)
   {
      ...
      if(!dtl::is_input_iterator<InputIt>::value &&
         free_cap < (n = static_cast<size_type>(lslboost::container::iterator_distance(first, last)))){
         ...
         lslboost::movelib::adaptive_merge(raw_beg, raw_pos, raw_end, comp, raw_end, free_cap - n);
      }
   }
   ...
   BOOST_CONTAINER_FORCEINLINE dtl::insert_range_proxy<allocator_type, lslboost::move_iterator<T*>, T*> priv_dummy_empty_proxy()
   {
      return dtl::insert_range_proxy<allocator_type, lslboost::move_iterator<T*>, T*>
   ...
   void priv_destroy_last_n(const size_type n) BOOST_NOEXCEPT_OR_NOTHROW
   {
      ...
      lslboost::container::destroy_alloc_n(this->get_stored_allocator(), this->priv_raw_end() - n, n);
      this->m_holder.dec_stored_size(n);
   }
   ...
   {
      return this->emplace(p, ::lslboost::forward<U>(u));
   }

   template <class U>
   BOOST_CONTAINER_FORCEINLINE void priv_push_back(BOOST_FWD_REF(U) u)
   {
      this->emplace_back(::lslboost::forward<U>(u));
   }

   //Overload to support compiler errors that instantiate too much
   BOOST_CONTAINER_FORCEINLINE void priv_push_back(::lslboost::move_detail::nat)
   {}

   BOOST_CONTAINER_FORCEINLINE iterator priv_insert(const_iterator, ::lslboost::move_detail::nat)
   {  return iterator();  }
      ...
      (T *const raw_pos, const size_type n, const InsertionProxy insert_range_proxy, version_1)
   {
      ...
      //Pass the hint so that allocators can take advantage of this.
      T * const new_buf = lslboost::movelib::to_raw_pointer(this->m_holder.allocate(new_cap));
      #ifdef BOOST_CONTAINER_VECTOR_ALLOC_STATS
      ++this->num_alloc;
      #endif
      ...
      (const pointer &pos, const size_type n, const InsertionProxy insert_range_proxy)
   {
      ...
      T *const p = lslboost::movelib::to_raw_pointer(pos);
      //Check if we have enough memory or try to expand current memory
      ...
      (T* const raw_pos, const size_type, InsertionProxy insert_range_proxy, dtl::true_type)
   {
      ...
      if (old_finish == raw_pos){
         ...
         allocator_traits_type::construct(a, old_finish, ::lslboost::move(*before_old_finish));
         ++this->m_holder.m_size;
         //Copy previous to last objects to the initialized end
         lslboost::container::move_backward(raw_pos, before_old_finish, old_finish);
   ...
   {
      //There is enough memory
      lslboost::container::expand_forward_and_insert_alloc
      ...
      (T* const new_start, size_type new_cap, T* const pos, const size_type n, InsertionProxy insert_range_proxy)
   {
      ...
      lslboost::container::uninitialized_move_and_insert_alloc
      ...
      if(raw_old_buffer){
         BOOST_IF_CONSTEXPR(!has_trivial_destructor_after_move<value_type>::value)
            lslboost::container::destroy_alloc_n(a, raw_old_buffer, this->m_holder.m_size);
          ...
          T* const pos, const size_type n, InsertionProxy insert_range_proxy)
   {
      ...
      if(s_before >= before_plus_new){
         ...
         T *const new_elem_pos =
            ::lslboost::container::uninitialized_move_alloc(a, old_start, pos, new_start);
         ...
         if(s_before >= new_size){
            ...
            if(before_plus_new != new_size){ //Special case to avoid operations in back insertion
               ::lslboost::container::uninitialized_move_alloc(a, pos, old_finish, new_start + before_plus_new);
         ...
         //s_before is so big that divides old_end
            ...
            BOOST_IF_CONSTEXPR(!value_traits::trivial_dctr){
               ...
               ::lslboost::container::uninitialized_move_alloc_n(a, pos, raw_gap, new_start + before_plus_new);
         ...
         if (s_before <= elemsbefore) {
            ...
            ::lslboost::container::uninitialized_move_alloc_n(a, old_start, s_before, new_start);
            ...
            if(do_after){
               ...
               BOOST_IF_CONSTEXPR(!value_traits::trivial_dctr_after_move){
                  lslboost::container::destroy_alloc_n(a, move_end, n_destroy);
               }
         ...
         if(do_after){
            ...
            if (elemsafter >= n_after){
               ...
               ::lslboost::container::uninitialized_move_alloc(a, finish_n, old_finish, old_finish);
               ...
               ::lslboost::container::uninitialized_move_alloc(a, pos, old_finish, old_finish + mid_last_dist);
...
struct has_trivial_destructor_after_move<lslboost::container::vector<T, Allocator, Options> >
{
   typedef typename lslboost::container::vector<T, Allocator, Options>::allocator_type allocator_type;
   typedef typename ::lslboost::container::allocator_traits<allocator_type>::pointer pointer;
   static const bool value = ::lslboost::has_trivial_destructor_after_move<allocator_type>::value &&

...
{
   (dispatch_uses_allocator)(construct_alloc, arg_alloc, dtl::addressof(p->first), ::lslboost::forward<U>(x));
   BOOST_TRY{
      (dispatch_uses_allocator)(construct_alloc, arg_alloc, dtl::addressof(p->second), ::lslboost::forward<V>(y));
   }
   BOOST_CATCH(...){
      allocator_traits<ConstructAlloc>::destroy(construct_alloc, dtl::addressof(p->first));
      BOOST_RETHROW
...
typename dtl::enable_if_and
   < void
   , dtl::is_pair<Pair>
   , dtl::not_<lslboost::move_detail::is_reference<Pair2> > >::type //This is needed for MSVC10 and ambiguous overloads
   dispatch_uses_allocator
   (ConstructAlloc & construct_alloc
      , BOOST_FWD_REF(ArgAlloc) arg_alloc
...
//piecewise construction from Std Tuple
   ...
   {
      (void)t1; (void)t2;
      (dispatch_uses_allocator)(construct_alloc, arg_alloc, dtl::addressof(pair->first), ::lslboost::forward<Args1>(get<Indexes1>(t1))...);
      BOOST_TRY{
         (dispatch_uses_allocator)(construct_alloc, arg_alloc, dtl::addressof(pair->second), ::lslboost::forward<Args2>(get<Indexes2>(t2))...);
      }
      BOOST_CATCH(...){
         allocator_traits<ConstructAlloc>::destroy(construct_alloc, dtl::addressof(pair->first));
         BOOST_RETHROW
...
{
   (dispatch_uses_allocator)(construct_alloc, arg_alloc, dtl::addressof(p->first), ::lslboost::forward<KeyType>(k));
   BOOST_TRY{
      (dispatch_uses_allocator)(construct_alloc, arg_alloc, dtl::addressof(p->second), ::lslboost::forward<Args>(args)...);
   }
   BOOST_CATCH(...) {
      allocator_traits<ConstructAlloc>::destroy(construct_alloc, dtl::addressof(p->first));
      BOOST_RETHROW

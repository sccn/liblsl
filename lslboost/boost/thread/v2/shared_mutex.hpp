...
namespace lslboost {
  namespace thread_v2 {
...
#ifdef BOOST_THREAD_USES_CHRONO
      template <class Rep, class Period>
      bool try_lock_shared_for(const lslboost::chrono::duration<Rep, Period>& rel_time)
      {
    ...
    inline void shared_mutex::lock()
    {
      ...
      gate1_.wait(lk, lslboost::bind(&shared_mutex::no_writer, lslboost::ref(*this)));
      state_ |= write_entered_;
      gate2_.wait(lk, lslboost::bind(&shared_mutex::no_readers, lslboost::ref(*this)));
    }
    ...
    {
      lslboost::unique_lock<mutex_t> lk(mut_);
      if (!gate1_.wait_until(lk, abs_time, lslboost::bind(
            &shared_mutex::no_writer, lslboost::ref(*this))))
      {
        return false;
      }
      state_ |= write_entered_;
      if (!gate2_.wait_until(lk, abs_time, lslboost::bind(
            &shared_mutex::no_readers, lslboost::ref(*this))))
      {
        state_ &= ~write_entered_;
        return false;
      }
    ...
    {
      lslboost::unique_lock<mutex_t> lk(mut_);
      if (!gate1_.timed_wait(lk, abs_or_rel_time, lslboost::bind(
            &shared_mutex::no_writer, lslboost::ref(*this))))
      {
        return false;
      }
      state_ |= write_entered_;
      if (!gate2_.timed_wait(lk, abs_or_rel_time, lslboost::bind(
            &shared_mutex::no_readers, lslboost::ref(*this))))
      {
        state_ &= ~write_entered_;
        return false;
    ...
    {
      lslboost::unique_lock<mutex_t> lk(mut_);
      gate1_.wait(lk, lslboost::bind(&shared_mutex::no_writer_no_max_readers, lslboost::ref(*this)));
        ...
        const lslboost::chrono::time_point<Clock, Duration>& abs_time)
    {
      lslboost::unique_lock<mutex_t> lk(mut_);
      if (!gate1_.wait_until(lk, abs_time, lslboost::bind(
            &shared_mutex::no_writer_no_max_readers, lslboost::ref(*this))))
      {
        return false;
      }
      count_t num_readers = (state_ & n_readers_) + 1;
...
namespace lslboost {
  namespace thread_v2 {
    ...
    {
      lslboost::unique_lock<mutex_t> lk(mut_);
      gate1_.wait(lk, lslboost::bind(&upgrade_mutex::no_writer_no_upgrader, lslboost::ref(*this)));
      state_ |= write_entered_;
      gate2_.wait(lk, lslboost::bind(&upgrade_mutex::no_readers, lslboost::ref(*this)));
    }
        ...
        const lslboost::chrono::time_point<Clock, Duration>& abs_time)
    {
      lslboost::unique_lock<mutex_t> lk(mut_);
      if (!gate1_.wait_until(lk, abs_time, lslboost::bind(
            &upgrade_mutex::no_writer_no_upgrader, lslboost::ref(*this))))
      {
        return false;
      }
      state_ |= write_entered_;
      if (!gate2_.wait_until(lk, abs_time, lslboost::bind(
            &upgrade_mutex::no_readers, lslboost::ref(*this))))
      {
        state_ &= ~write_entered_;
        return false;
      }
    ...
    {
      lslboost::unique_lock<mutex_t> lk(mut_);
      gate1_.wait(lk, lslboost::bind(&upgrade_mutex::no_writer_no_max_readers, lslboost::ref(*this)));
      count_t num_readers = (state_ & n_readers_) + 1;
      state_ &= ~n_readers_;
      state_ |= num_readers;
    }
        ...
        const lslboost::chrono::time_point<Clock, Duration>& abs_time)
    {
      lslboost::unique_lock<mutex_t> lk(mut_);
      if (!gate1_.wait_until(lk, abs_time, lslboost::bind(
            &upgrade_mutex::no_writer_no_max_readers, lslboost::ref(*this))))
      {
        return false;
      }
      count_t num_readers = (state_ & n_readers_) + 1;
    ...
    {
      lslboost::unique_lock<mutex_t> lk(mut_);
      gate1_.wait(lk, lslboost::bind(&upgrade_mutex::no_writer_no_upgrader_no_max_readers, lslboost::ref(*this)));
      count_t num_readers = (state_ & n_readers_) + 1;
      state_ &= ~n_readers_;
        ...
        const lslboost::chrono::time_point<Clock, Duration>& abs_time)
    {
      lslboost::unique_lock<mutex_t> lk(mut_);
      if (!gate1_.wait_until(lk, abs_time, lslboost::bind(
            &upgrade_mutex::no_writer_no_upgrader_no_max_readers, lslboost::ref(*this))))
      {
        return false;
      }
      count_t num_readers = (state_ & n_readers_) + 1;

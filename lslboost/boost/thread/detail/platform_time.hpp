...
namespace lslboost
{
//typedef lslboost::int_least64_t time_max_t;
typedef lslboost::intmax_t time_max_t;
...
inline FP init_steady_clock(kern_return_t & err)
{
  ...
  namespace detail
  {
    ...
    inline timespec ns_to_timespec(lslboost::time_max_t const& ns)
    {
      lslboost::time_max_t s = ns / 1000000000l;
      timespec ts;
      ts.tv_sec = static_cast<long> (s);
      ts.tv_nsec = static_cast<long> (ns - s * 1000000000l);
      return ts;
    }
    inline lslboost::time_max_t timespec_to_ns(timespec const& ts)
    {
      return static_cast<lslboost::time_max_t>(ts.tv_sec) * 1000000000l + ts.tv_nsec;
    }
#endif
    ...
    struct platform_duration
    {
      ...
      explicit platform_duration(lslboost::time_max_t const& ns = 0) : ts_val(ns_to_timespec(ns)) {}
      lslboost::time_max_t getNs() const { return timespec_to_ns(ts_val); }
#else
      explicit platform_duration(lslboost::time_max_t const& ns = 0) : ns_val(ns) {}
      lslboost::time_max_t getNs() const { return ns_val; }
#endif

#if defined BOOST_THREAD_USES_DATETIME
      platform_duration(lslboost::posix_time::time_duration const& rel_time)
      {
#if defined BOOST_THREAD_CHRONO_POSIX_API || defined BOOST_THREAD_CHRONO_MAC_API
...
#else
        ns_val = static_cast<lslboost::time_max_t>(rel_time.total_seconds()) * 1000000000l;
      ...
      lslboost::time_max_t getMs() const
      {
        const lslboost::time_max_t ns = getNs();
    ...
    struct real_platform_timepoint
    {
      ...
      explicit real_platform_timepoint(lslboost::time_max_t const& ns) : dur(ns) {}
      lslboost::time_max_t getNs() const { return dur.getNs(); }

#if defined BOOST_THREAD_USES_DATETIME
      real_platform_timepoint(lslboost::system_time const& abs_time)
        : dur(abs_time - lslboost::posix_time::from_time_t(0)) {}
#endif
  ...
  struct mono_platform_timepoint
  {
    ...
    explicit mono_platform_timepoint(lslboost::time_max_t const& ns) : dur(ns) {}
    lslboost::time_max_t getNs() const { return dur.getNs(); }
...
#else
      lslboost::time_max_t ns = (std::numeric_limits<lslboost::time_max_t>::max)();
  ...
  struct mono_platform_clock
  {
    static mono_platform_timepoint now()
    {
      ...
      return mono_platform_timepoint(static_cast<lslboost::time_max_t>(ns));
#else

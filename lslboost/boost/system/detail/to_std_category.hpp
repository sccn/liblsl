...
public:
    ...
    explicit std_category( lslboost::system::error_category const * pc, unsigned id ): pc_( pc )
    {
        if( id != 0 )
        {
...
inline std::error_category const & to_std_category( lslboost::system::error_category const & cat ) BOOST_SYMBOL_VISIBLE;
#endif

struct cat_ptr_less
{
    bool operator()( lslboost::system::error_category const * p1, lslboost::system::error_category const * p2 ) const BOOST_NOEXCEPT
    {
        return *p1 < *p2;
    }
};

inline std::error_category const & to_std_category( lslboost::system::error_category const & cat )
{
    if( cat.id_ == lslboost::system::detail::system_category_id )
    {
        static const std_category system_instance( &cat, 0x1F4D7 );
        return system_instance;
    }
    else if( cat.id_ == lslboost::system::detail::generic_category_id )
    {
    ...
    {
        typedef std::map< lslboost::system::error_category const *, std::unique_ptr<std_category>, cat_ptr_less > map_type;
...
inline bool std_category::equivalent( int code, const std::error_condition & condition ) const BOOST_NOEXCEPT
{
    ...
    else if( condition.category() == std::generic_category() || condition.category() == lslboost::system::generic_category() )
    {
        lslboost::system::error_condition bn( condition.value(), lslboost::system::generic_category() );
        return pc_->equivalent( code, bn );
    }
...
inline bool std_category::equivalent( const std::error_code & code, int condition ) const BOOST_NOEXCEPT
{
    ...
    else if( code.category() == std::generic_category() || code.category() == lslboost::system::generic_category() )
    {
        lslboost::system::error_code bc( code.value(), lslboost::system::generic_category() );
        return pc_->equivalent( bc, condition );
    }

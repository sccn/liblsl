...
std::size_t find_address_dispatch(const volatile void* addr, const volatile void* const* addrs, std::size_t size)
{
    ...
    func_ptr_operations::store(g_find_address.as_storage, ptr.as_storage, lslboost::memory_order_relaxed);
...
{
    find_address_ptr ptr;
    ptr.as_storage = func_ptr_operations::load(g_find_address.as_storage, lslboost::memory_order_relaxed);
...
struct lock_state
{
    ...
    void long_lock() BOOST_NOEXCEPT
    {
        ...
        {
            futex_operations::storage_type prev_state = futex_operations::load(m_mutex, lslboost::memory_order_relaxed);
            if (BOOST_LIKELY((prev_state & mutex_bits::locked) == 0u))
            {
                futex_operations::storage_type new_state = prev_state | mutex_bits::locked;
                if (BOOST_LIKELY(futex_operations::compare_exchange_strong(m_mutex, prev_state, new_state, lslboost::memory_order_acquire, lslboost::memory_order_relaxed)))
                    return;
            }
    ...
    void lock_slow_path() BOOST_NOEXCEPT
    {
        ...
        while (true)
        {
            ...
            {
                futex_operations::storage_type new_state = prev_state | mutex_bits::locked;
                if (BOOST_LIKELY(futex_operations::compare_exchange_weak(m_mutex, prev_state, new_state, lslboost::memory_order_acquire, lslboost::memory_order_relaxed)))
                    return;
            }
            else
            {
                futex_operations::storage_type new_state = prev_state | mutex_bits::contended;
                if (BOOST_LIKELY(futex_operations::compare_exchange_weak(m_mutex, prev_state, new_state, lslboost::memory_order_relaxed, lslboost::memory_order_relaxed)))
                {
                    atomics::detail::futex_wait_private(&m_mutex, new_state);
                    prev_state = futex_operations::load(m_mutex, lslboost::memory_order_relaxed);
                }
            }
        }
    }
    ...
    void unlock() BOOST_NOEXCEPT
    {
        ...
        while (true)
        {
            ...
            if (BOOST_LIKELY(futex_operations::compare_exchange_weak(m_mutex, prev_state, new_state, lslboost::memory_order_release, lslboost::memory_order_relaxed)))
                break;
        }
        ...
        if ((prev_state & mutex_bits::contended) != 0u)
        {
            ...
            if (woken_count == 0)
            {
                ...
                futex_operations::compare_exchange_strong(m_mutex, prev_state, new_state, lslboost::memory_order_relaxed, lslboost::memory_order_relaxed);
            }
        }
    }
};
...
inline void wait_state::notify_one(lock_state& state) BOOST_NOEXCEPT
{
    ...
    if (BOOST_LIKELY(m_waiter_count > 0u))
    {
        ...
        futex_extra_operations::opaque_or(state.m_mutex, mutex_bits::contended, lslboost::memory_order_relaxed);
    }
}
...
{
    //! Condition variable
    lslboost::winapi::CONDITION_VARIABLE_ m_cond;

    explicit wait_state(std::size_t index) BOOST_NOEXCEPT :
        wait_state_base(index)
    {
        lslboost::winapi::InitializeConditionVariable(&m_cond);
    }
    ...
    void notify_one(lock_state&) BOOST_NOEXCEPT
    {
        lslboost::winapi::WakeConditionVariable(&m_cond);
    }
    //! Wakes up all threads blocked in the wait operation
    void notify_all(lock_state&) BOOST_NOEXCEPT
    {
        lslboost::winapi::WakeAllConditionVariable(&m_cond);
    }
};
...
{
    //! Mutex
    lslboost::winapi::SRWLOCK_ m_mutex;
    //! Wait states
    wait_state_list m_wait_states;
    ...
    void long_lock() BOOST_NOEXCEPT
    {
        ...
        lslboost::winapi::AcquireSRWLockExclusive(&m_mutex);
    }

    //! Unlocks the mutex
    void unlock() BOOST_NOEXCEPT
    {
        lslboost::winapi::ReleaseSRWLockExclusive(&m_mutex);
    }
};
...
inline void wait_state::wait(lock_state& state) BOOST_NOEXCEPT
{
    lslboost::winapi::SleepConditionVariableSRW(&m_cond, &state.m_mutex, lslboost::winapi::infinite, 0u);
}
    ...
    public wait_state_base
{
    ...
    struct semaphore
    {
        ...
        //! Semaphore handle
        lslboost::winapi::HANDLE_ m_semaphore;
        //! Number of threads blocked on the semaphore
        lslboost::winapi::ULONG_ m_waiter_count;
        //! Number of threads released by notifications
        lslboost::winapi::ULONG_ m_notify_count;

        semaphore() BOOST_NOEXCEPT :
            m_semaphore(lslboost::winapi::create_anonymous_semaphore(NULL, 0, (std::numeric_limits< lslboost::winapi::LONG_ >::max)())),
            m_waiter_count(0u),
            m_notify_count(0u)
        {
            m_next = m_prev = this;
        ...
        ~semaphore() BOOST_NOEXCEPT
        {
            ...
            if (BOOST_LIKELY(m_semaphore != lslboost::winapi::invalid_handle_value))
                lslboost::winapi::CloseHandle(m_semaphore);
        }
        ...
        static semaphore* create() BOOST_NOEXCEPT
        {
            ...
            if (BOOST_UNLIKELY(p != NULL && p->m_semaphore == lslboost::winapi::invalid_handle_value))
            {
                delete p;
                p = NULL;
            }
    ...
    void notify_one(lock_state&) BOOST_NOEXCEPT
    {
        ...
        lslboost::winapi::ReleaseSemaphore(sem->m_semaphore, 1, NULL);
    }
    ...
    void notify_all(lock_state&) BOOST_NOEXCEPT
    {
        ...
        if (sem)
        {
            while (true)
            {
                ...
                lslboost::winapi::ULONG_ count = sem->m_waiter_count - sem->m_notify_count;
                sem->m_notify_count += count;

                lslboost::winapi::ReleaseSemaphore(sem->m_semaphore, count, NULL);
...
{
    //! Mutex
    lslboost::winapi::CRITICAL_SECTION_ m_mutex;
    ...
    void long_lock() BOOST_NOEXCEPT
    {
        mutex_operations::storage_type fallback_state = mutex_operations::load(m_mutex_fallback, lslboost::memory_order_relaxed);
        while (true)
        {
            if (BOOST_LIKELY(fallback_state == fallback_mutex_bits::critical_section_initialized))
            {
            lock_cs:
                lslboost::winapi::EnterCriticalSection(&m_mutex);
                return;
            }

            while (fallback_state == 0u)
            {
                if (!mutex_operations::compare_exchange_weak(m_mutex_fallback, fallback_state, fallback_mutex_bits::locked, lslboost::memory_order_acquire, lslboost::memory_order_relaxed))
                    continue;

                if (BOOST_LIKELY(!!lslboost::winapi::InitializeCriticalSectionAndSpinCount(&m_mutex, 100u)))
                {
                    mutex_operations::store(m_mutex_fallback, fallback_mutex_bits::critical_section_initialized, lslboost::memory_order_release);
                    goto lock_cs;
                }
            ...
            if (fallback_state == fallback_mutex_bits::locked)
            {
                ...
                lslboost::winapi::SwitchToThread();
                fallback_state = mutex_operations::load(m_mutex_fallback, lslboost::memory_order_relaxed);
            }
        }
    }
    ...
    void unlock() BOOST_NOEXCEPT
    {
        ...
        if (BOOST_LIKELY(fallback_state == fallback_mutex_bits::critical_section_initialized))
        {
            lslboost::winapi::LeaveCriticalSection(&m_mutex);
            return;
        }

        mutex_operations::store(m_mutex_fallback, 0u, lslboost::memory_order_release);
    }
};
...
inline void wait_state::wait(lock_state& state) BOOST_NOEXCEPT
{
    ...
    if (sem)
    {
        while (sem->m_waiter_count >= static_cast< lslboost::winapi::ULONG_ >((std::numeric_limits< lslboost::winapi::LONG_ >::max)()))
        {
    ...
    lslboost::winapi::WaitForSingleObject(sem->m_semaphore, lslboost::winapi::infinite);
    ...
    else
    {
        ...
        if (BOOST_UNLIKELY(m_header->size == m_header->capacity))
        {
            ...
            if (BOOST_UNLIKELY(new_header == NULL))
                return NULL;
            lslboost::alignment::aligned_free(static_cast< void* >(m_header));
            m_header = new_header;
        }
    }
...
{
    if (BOOST_UNLIKELY(once_flag_operations::load(g_pool_cleanup_registered.m_flag, lslboost::memory_order_relaxed) == 0u))
    {
        if (once_flag_operations::exchange(g_pool_cleanup_registered.m_flag, 1u, lslboost::memory_order_relaxed) == 0u)
            std::atexit(&cleanup_lock_pool);
    }

    const std::size_t new_buffer_size = entries_offset + new_capacity * sizeof(void*) * 2u;

    void* p = lslboost::alignment::aligned_alloc(buffer_alignment, new_buffer_size);

...
BOOST_ATOMIC_DECL void initialize_wait_functions() BOOST_NOEXCEPT
{
    ...
    once_flag_operations::storage_type old_val = once_flag_operations::load(wait_functions_once_flag.m_flag, lslboost::memory_order_acquire);
    while (true)
    {
        if (old_val == 2u)
        {
            if (BOOST_UNLIKELY(!once_flag_operations::compare_exchange_strong(wait_functions_once_flag.m_flag, old_val, 1u, lslboost::memory_order_relaxed, lslboost::memory_order_relaxed)))
                continue;

            lslboost::winapi::HMODULE_ kernel_base = lslboost::winapi::get_module_handle(L"api-ms-win-core-synch-l1-2-0.dll");
            if (BOOST_LIKELY(kernel_base != NULL))
            {
                wait_on_address_t* woa = (wait_on_address_t*)lslboost::winapi::get_proc_address(kernel_base, "WaitOnAddress");
                if (BOOST_LIKELY(woa != NULL))
                {
                    wake_by_address_t* wbas = (wake_by_address_t*)lslboost::winapi::get_proc_address(kernel_base, "WakeByAddressSingle");
                    wake_by_address_t* wbaa = (wake_by_address_t*)lslboost::winapi::get_proc_address(kernel_base, "WakeByAddressAll");
            ...
            once_flag_operations::store(wait_functions_once_flag.m_flag, 0u, lslboost::memory_order_release);
            break;
        }
        else if (old_val == 1u)
        {
            lslboost::winapi::SwitchToThread();
            old_val = once_flag_operations::load(wait_functions_once_flag.m_flag, lslboost::memory_order_acquire);
        }
        else
        {
            break;
...
BOOST_ATOMIC_DECL wait_on_address_t* wait_on_address = &lslboost::winapi::WaitOnAddress;
BOOST_ATOMIC_DECL wake_by_address_t* wake_by_address_single = &lslboost::winapi::WakeByAddressSingle;
BOOST_ATOMIC_DECL wake_by_address_t* wake_by_address_all = &lslboost::winapi::WakeByAddressAll;

 ...
 * than the number of channels in the sample. Each entry in data must be longer than the corresponding
 * entry in `lengths`.
 * @param lengths An array containing the lengths of each buffer in data. Units are string lengths or number of bytes.
 */
 ...
 * @param data An array of data buffers to push. The number of buffers in the array must be no less than `nbufs`.
 * @param bytes An array comprising the number of bytes in each buffer. The number of entries in bytes must be no less than `nbufs`.
 * @param nbufs The number of values pointed to in `data` and equivalently the number of items in `bytes`.
 */
extern LIBLSL_C_API int32_t lsl_push_sample_rawtpn(lsl_outlet out, void **data, const uint32_t *bytes, double timestamp, int32_t pushthrough, uint32_t nbufs);
